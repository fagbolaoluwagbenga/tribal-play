...
var options = {
    auth: {
      api_key:
        "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"
    }
  };
                                ...
                                if (err) {
                                    Plan.find({}, function (err, allPlans) {
                                        if (err) {
                                            ...
                                            res.render("signup-step1", { plans: allPlans });
                                        }
                                    });
...
// login mobile
exports.get_login_routeM = function (req, res) {
        res.render("loginM");
}
    ...
    async.waterfall([
      ...
      function(user, done) {
        var smtpTransport = nodemailer.createTransport({
          service: 'Gmail', 
          auth: {
            user: 'tribalplay.io@gmail.com',
            pass: process.env.GMAILPW
          }
        });
        var mailOptions = {
          to: user.email,
          from: 'tribalplay.io@gmail.com',
...
exports.get_user_id =  function(req, res) {
    User.findById(req.params.id, function(err, foundUser) {
      ...
      NewMovie.find().where('author.id').equals(foundUser._id).exec(function(err, newMovies) {
        if(err) {
          req.flash("error", "Something went wrong.");
          res.redirect("/");
        }
        res.render("/dashboard", {user: foundUser, newMovies: newMovies});
      })
    });
  }
